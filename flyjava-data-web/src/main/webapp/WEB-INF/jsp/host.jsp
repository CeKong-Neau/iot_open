...
<body class="fixed-sidebar">
	...
	<div class="row white-bg dashboard-header">
		...
		<c:forEach items="${hostList}" var="item">
			...
			<a href="<%=DATA_URL%>/data/showUserProduct?hostId=${item.hostId }" >
				<div class="col-sm-6">
						<div class="panel panel-info">
							<div class="panel-body bg-info">
								<h4 >主机名称:${item.hostName}&nbsp;&nbsp;&nbsp;&nbsp;<em>No.${item.hostId }</em></h4>
				    		...
				    		<table class="table table-responsive">
				    				...
				    				<tr>
				    					...
				    					<td><c:if test="${item.status eq 1}"> <small class="text-success">工作中</small> </c:if>
				    						<c:if test="${item.status eq 0}"> <small class="text-danger">故障</small></c:if> </td>
				    				</tr>
				    		</table>
				    	</div>
				</div>
		...
		var options = {
		    ...
		    pageUrl: function(type, page, current){
		            if (page==current) {
		                ...
		                return "<%=DATA_URL%>"+"/data/showUserHost?page="+page;
		            }
		        }
		}
